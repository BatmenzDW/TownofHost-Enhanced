...
internal class Baker : RoleBase
{
    ...
    public override string GetMarkOthers(PlayerControl seer, PlayerControl target, bool isForMeeting = false)
    {
        ...
        if (HasBread(_Player.PlayerId, target.PlayerId) && seer.IsNeutralApocalypse() && seer.PlayerId != _Player.PlayerId && !Main.PlayerStates[seer.PlayerId].IsFalseRole)
        {
    ...
    public override bool ForcedCheckMurderAsKiller(PlayerControl killer, PlayerControl target)
    {
        ...
        else if (target.IsNeutralApocalypse() && !Main.PlayerStates[target.PlayerId].IsFalseRole)
    ...
    public override void OnFixedUpdate(PlayerControl player, bool lowLoad, long nowTime, int timerLowLoad)
    {
        ...
        if (AllHasBread(player) || (TransformNoMoreBread.GetBool() && BreadedPlayerCount(player.PlayerId).Item1 >= Main.EnumerateAlivePlayerControls().Count(x => !x.IsNeutralApocalypse() && !Main.PlayerStates[x.PlayerId].IsFalseRole)))
        {
...
internal class Famine : RoleBase
{
    ...
    public override bool ForcedCheckMurderAsKiller(PlayerControl killer, PlayerControl target)
    {
        if (target.IsNeutralApocalypse() && !Main.PlayerStates[target.PlayerId].IsFalseRole)
    ...
    public override void OnCheckForEndVoting(PlayerState.DeathReason deathReason, params byte[] exileIds)
    {
        ...
        foreach (var pc in Main.EnumerateAlivePlayerControls())
        {
            if ((pc.IsNeutralApocalypse() && !Main.PlayerStates[pc.PlayerId].IsFalseRole) || Baker.HasBread(baker.PlayerId, pc.PlayerId)) continue;
            if (baker.IsAlive())
            {
