...
internal class CovenLeader : CovenManager
{
    ...
    private static bool IsHelper(PlayerControl covenLeader, PlayerControl target)
    {
        ...
        return target.Is(CustomRoles.Enchanted) || (target.Is(CustomRoles.Romantic) && Romantic.BetPlayer.TryGetValue(target.PlayerId, out var romanticTarget) && covenList.Contains(GetPlayerById(romanticTarget))) || (target.Is(CustomRoles.Lawyer) && covenList.Any(x => Lawyer.TargetList.Contains(x.PlayerId))) || (target.Is(CustomRoles.Medic) && covenList.Any(x => Medic.IsProtected(x.PlayerId))) || covenList.Any(x => Lovers.AreLovers(x, target)) || (target.Is(CustomRoles.Monarch) && covenList.Any(x => x.Is(CustomRoles.Knighted))) || (target.Is(CustomRoles.Follower) && Follower.BetPlayer.TryGetValue(target.PlayerId, out var followerTarget) && covenList.Contains(GetPlayerById(followerTarget))) || (target.Is(CustomRoles.SchrodingersCat) && SchrodingersCat.teammate.TryGetValue(target.PlayerId, out var catTeammate) && covenList.Contains(GetPlayerById(catTeammate))) || (target.Is(CustomRoles.Crusader) && target.GetRoleClass() is Crusader crus && covenList.Any(x => crus.ForCrusade.Contains(x.PlayerId))) ||
