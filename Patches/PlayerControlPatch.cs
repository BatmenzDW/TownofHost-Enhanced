...
class MurderPlayerPatch
{
    ...
    public static void Postfix(PlayerControl __instance, [HarmonyArgument(0)] PlayerControl target/*, [HarmonyArgument(1)] MurderResultFlags resultFlags*/, bool __state)
    {
        ...
        if (Main.FirstDied == "")
        {
            ...
            {
                LegacyRpcSenders.SendSyncShieldPersonDiedFirst(PlayerControl.LocalPlayer.NetId, Main.FirstDied, Main.FirstDiedPrevious);
            }
        }
                ...
                foreach (var subRole in playerSubRoles)
                {
                    switch (subRole)
                    {
                        ...
                        case CustomRoles.Madmate when taskState.IsTaskFinished && player.Is(CustomRoles.Snitch):
                            foreach (var impostor in Main.EnumerateAlivePlayerControls().Where(pc => pc.Is(Custom_Team.Impostor) && !Main.PlayerStates[pc.PlayerId].IsFalseRole).ToArray())
                            {
