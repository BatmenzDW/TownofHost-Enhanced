...
public class SetUpRoleTextPatch
{
    ...
    private static IEnumerator CoLoggerGameInfo()
    {
        ...
        else
        {
            ...
            try
            {
                ...
                byte[] encryptedBytes = EncryptDES(logBytes, $"BHR{PlayerControl.LocalPlayer.PlayerId}00000000"[..8]);
        ...
        //mode CBC, PKCS7, 64bit, Key = IV= "BHR" + playerid + 000/00 "to 8 bits
...
class BeginCrewmatePatch
{
    ...
    public static void Postfix(IntroCutscene __instance)
    {
        ...
        switch (role)
        {
            ...
            case CustomRoles.ShapeshifterBHR:
                PlayerControl.LocalPlayer.Data.Role.IntroSound = GetIntroSound(RoleTypes.Shapeshifter);
                break;
...
class BeginImpostorPatch
{
    public static bool Prefix(IntroCutscene __instance, ref Il2CppSystem.Collections.Generic.List<PlayerControl> yourTeam)
    {
            ...
            || PlayerControl.LocalPlayer.Is(CustomRoles.Narc))
        {
            yourTeam = new();
            yourTeam.Add(PlayerControl.LocalPlayer);
            foreach (var pc in Main.EnumerateAlivePlayerControls().Where(x => !x.AmOwner)) yourTeam.Add(pc);
            __instance.BeginCrewmate(yourTeam);
#if !ANDROID
