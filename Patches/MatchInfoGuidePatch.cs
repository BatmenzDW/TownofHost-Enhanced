...
// Credit: EHR: https://github.com/Gurge44/EndlessHostRoles/blob/main/Patches/MatchInfoGuidePatch.cs
[HarmonyPatch]
public static class MatchInfoGuidePatch
{
    ...
    public static bool CreateModdedSettings(MatchInfoGuide __instance)
    {
        ...
        CreateModdedSettingEntries([.. OptionItem.AllOptions.Where(x => x.GetBool() && x.Parent == null && x.Tab is TabGroup.ModSettings && !x.IsHiddenOn(Options.CurrentGameMode))], __instance, ref num);
        CreateModdedSettingEntries([.. OptionItem.AllOptions.Where(x => x.GetBool() && x.Parent == null && x.Tab is TabGroup.SystemSettings && !x.IsHiddenOn(Options.CurrentGameMode))], __instance, ref num);
    ...
    private static int CreateModdedRoleEntries(MatchInfoGuide __instance)
    {
        ...
        {
            if (!role.IsVanilla() && role.IsEnable())
            {
    ...
    private static void CreateModdedRoleEntry(MatchInfoGuide __instance, CustomRoles role)
    {
        ...
        string roleChance = Translator.GetString(Utils.GetColoredChanceText(role.GetMode()));
    ...
    private static RoleBehaviour GetRoleBehaviourFromBasis(CustomRoles role)
    {
        ...
        {
            CustomRoles.Doctor or CustomRoles.Medic => RoleTypes.Scientist,
