...
internal class DialogueBoxPatch
{
    ...
    {
        if (!PlayerControl.LocalPlayer.inVent && dialogue.Contains("<size=0%>BHR</size>") && GameStates.IsInTask)
        {
    ...
    {
        if (__instance.target.text.Contains("<size=0%>BHR</size>"))
        {
    ...
    /*
     * Lets add a <size=0%>BHR</size> at where you will use the dialogue box.
     */
}
