...
public static class StringOptionPatch
{
    ...
    private static bool InitializePrefix(StringOption __instance)
    {
        if (ModGameOptionsMenu.OptionList.TryGetValue(__instance.GetInstanceID(), out var index))
        {
            ...
            if (Main.CustomRoleValues.Find(x => GetString($"{x}") == name1.RemoveHtmlTags(), out var role))
            {
    ...
    private static void SetupHelpIcon(CustomRoles role, StringOption __instance)
    {
        ...
        GameOptionsButton.OnClick.AddListener((Action)(() =>
        {
            if (ModGameOptionsMenu.OptionList.TryGetValue(__instance.GetInstanceID(), out var index))
            {
                var item = OptionItem.AllOptions[index];
                var name = item.GetName();
                if (Main.CustomRoleValues.Find(x => GetString($"{x}") == name.RemoveHtmlTags(), out var role))
                {
                    var roleName = role.IsVanilla() ? role + "BHR" : role.ToString();
